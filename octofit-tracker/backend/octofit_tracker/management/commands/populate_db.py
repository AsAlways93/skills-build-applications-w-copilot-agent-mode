...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for user in users:
            user.save()
            self.stdout.write(f"Saved user: {user.username}")

        # Create teams
        team1 = Team(_id=ObjectId(), name='Blue Team')
        team2 = Team(_id=ObjectId(), name='Gold Team')
        team1.save()
        self.stdout.write(f"Saved team: {team1.name}")
        team2.save()
        self.stdout.write(f"Saved team: {team2.name}")
        ...
        for activity in activities:
            activity.save()
            self.stdout.write(f"Saved activity: {activity.activity_type}")
        ...
        for entry in leaderboard_entries:
            entry.save()
            self.stdout.write(f"Saved leaderboard entry for user: {entry.user.username}")
        ...
        for workout in workouts:
            workout.save()
            self.stdout.write(f"Saved workout: {workout.name}")
